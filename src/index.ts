...
const main = async () => {
  function getFiles(dir: fs.PathLike, files: any[] = []) {
  ...
  const filenames: string[] = getFiles(
    'C:\\Users\\yanis\\IdeaProjects\\TranspilerLS\\src\\editor',
    []
  );
  console.log(filenames);

  function getLastModifiedDate(filepath: fs.PathLike) {
    const stats = fs.statSync(filepath);

    return stats.mtime;
  }

  console.log(getLastModifiedDate(filenames[0]));
  ...
  */
  ...
  let code = concatFile(process.env.ABSOLUTE_PATH_TO_SOURCES!, [
    process.env.ABSOLUTE_PATH_TO_SOURCES!,
  ]);
  ...
  if (process.env.ENABLE_SAVE && process.env.FILE_NAME) {
    const rootFileId = await api.getAiId(process.env.FILE_NAME);
